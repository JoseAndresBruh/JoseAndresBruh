# 👋 ¡Hola! Soy José Andrés

<div align="center">

![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Architects+Daughter&color=7AF79A&size=35&center=true&vCenter=true&width=600&height=60&duration=2000&pause=1000&lines=Software+Engineering+Student+🎓;Backend+%26+Architecture+Enthusiast+💻;Problem+Solver+🧠;Building+Structured+Solutions+✨)

</div>

---

## 🌍 Sobre Mí

Soy estudiante de Ingeniería de Software (5to semestre) en la ULEAM. Me especializo en el desarrollo backend, el diseño de arquitecturas robustas y la implementación de soluciones escalables. Me enfoco en mantener un código limpio, estructurado y fácil de mantener a largo plazo.

- 🗣️ **Idiomas:** Inglés (Nivel B2) y Español (Nativo).
- 🌱 **Intereses actuales:** Profundizando en arquitecturas distribuidas, despliegue mediante contenedores y automatización.
- ⚡ **Enfoque:** Resolución de problemas lógicos complejos, optimización de procesos y trabajo colaborativo.

---

## 🏗️ Arquitectura y Metodologías

Para asegurar la calidad en mis proyectos, aplico principios sólidos de ingeniería tanto en la estructura del código como en la forma de trabajar:

- **Arquitecturas y Patrones:** Implementación de **Diseño Guiado por el Dominio (DDD)** para modelar lógicas de negocio complejas de manera escalable. También tengo experiencia en el despliegue de **arquitecturas distribuidas**, manejando replicación bidireccional entre distintos motores de bases de datos.
- **Metodologías de Trabajo:** Trabajo bajo un enfoque fuertemente **Ágil**. Aplico metodologías como **Scrum** y prácticas de **eXtreme Programming (XP)** para asegurar entregas continuas, iteraciones rápidas y alta adaptabilidad a los cambios.
- **Calidad de Código:** Integración de herramientas de análisis estático para mantener un estándar profesional y reducir la deuda técnica.

---

# 💻 Tech Stack & Herramientas

<div align="center">

### ⚙️ Lenguajes & Frameworks
![C#](https://img.shields.io/badge/c%23-%23239120.svg?style=for-the-badge&logo=csharp&logoColor=white) ![.NET](https://img.shields.io/badge/.NET-5C2D91?style=for-the-badge&logo=.net&logoColor=white) ![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) ![JavaScript](https://img.shields.io/badge/javascript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)

### 🗄️ Bases de Datos
![PostgreSQL](https://img.shields.io/badge/postgresql-%23316192.svg?style=for-the-badge&logo=postgresql&logoColor=white) ![MySQL](https://img.shields.io/badge/mysql-%2300f.svg?style=for-the-badge&logo=mysql&logoColor=white) ![SQL](https://img.shields.io/badge/sql-%23CC2927.svg?style=for-the-badge&logo=microsoftsqlserver&logoColor=white) ![Supabase](https://img.shields.io/badge/Supabase-181818?style=for-the-badge&logo=supabase&logoColor=3ECF8E)

### 🛠️ Entornos & Herramientas
![Visual Studio](https://img.shields.io/badge/Visual%20Studio-5C2D91.svg?style=for-the-badge&logo=visual-studio&logoColor=white) ![SonarQube](https://img.shields.io/badge/SonarQube-black?style=for-the-badge&logo=sonarqube&logoColor=4E9BCD) ![Docker](https://img.shields.io/badge/docker-%230db7ed.svg?style=for-the-badge&logo=docker&logoColor=white) ![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)

</div>

---

<div align="center">
<img alt="Bongo Cat" height="70px" width="70px" src="https://c.tenor.com/fYg91qBpDdgAAAAi/bongo-cat-transparent.gif"></img><br>

<a href="https://github.com/JoseAndresBruh">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=JoseAndresBruh&theme=github_dark&hide_border=true" alt="GitHub Stats Graph"/>
</a>
  
<br>

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=JoseAndresBruh&layout=compact&langs_count=6&theme=tokyonight)](https://github.com/JoseAndresBruh)

</div>

---

## 🌐 Contáctame

<div align="center">

[joseandrescorreodecontacto2@gmail.com](mailto:joseandrescorreodecontacto2@gmail.com)

<br><br>

### ¡Gracias por visitar mi perfil!

<br>
<img src="https://media.tenor.com/0vKzY9qT0YcAAAAi/pochita-chainsaw-man.gif" width="70" alt="Pochita" /> &nbsp; &nbsp; &nbsp; &nbsp; <img src="https://media.tenor.com/R8pP6k_6vLIAAAAi/spamton-spamton-g-spamton.gif" width="60" alt="Spamton" />
<br>

<p>
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=65&section=footer"/>
</p>
</div>
